...
class TReportController extends Controller
{
    ...
    public function index(){
        ...
        return view('workshop-ms.t-workshop.index',compact('bays', 'baysT', 'sectionT', 'workshop', 'scl', 'reason', 'part','CUnitTICJ','CUnitTEJ','CUnitTICC','CUnitTEC','CUnitTRT','CUnitBTRT','CUnitBTS','CUnitRTR','CUnitRS','CUnitST','CUnitPPT','CUnitOPC','CUnitHPT','CUnitTotal'));
    }
    ...
    public function indexR(){
        ...
        $bnunit = DB::SELECT('SELECT * FROM unit_pull_outs WHERE POUStatus="" AND POUTransferArea="" AND POUTransferBay="" AND isBrandNew=1 AND POUBrand=1');
        
        $pounit = DB::SELECT('SELECT * FROM unit_pull_outs WHERE POUStatus="" AND POUTransferArea="" AND POUTransferBay="" AND isBrandNew=0 AND POUBrand=1');
        ...
        $cunit = DB::SELECT('SELECT unit_confirms.id, unit_confirms.POUID, unit_confirms.CUTransferDate, unit_confirms.CUTransferRemarks, unit_confirms.CUTransferStatus, unit_confirms.CUTransferArea, unit_confirms.CUTransferBay,
                            ...
                            WHERE unit_confirms.CUDelTransfer=0 AND unit_pull_outs.POUBrand = 1
                            ');
    ...
    public function sortPullOut(Request $request){
        ...
        if($UStatus == 'pouRadioPOU'){
            $pounit = DB::SELECT('SELECT * FROM unit_pull_outs WHERE POUStatus="" AND isBrandNew=0 AND POUBrand=1');
        }else if($UStatus == 'pouRadioCU'){
            $pounit = DB::SELECT('SELECT * FROM unit_pull_outs INNER JOIN unit_confirms ON unit_pull_outs.id = unit_confirms.POUID WHERE isBrandNew=0 AND POUBrand=1 AND CUDelTransfer=0');
        }else if($UStatus == 'pouRadioDU'){
            $pounit = DB::SELECT('SELECT * FROM unit_pull_outs INNER JOIN unit_deliveries ON unit_pull_outs.id = unit_deliveries.POUID WHERE isBrandNew=0 AND POUBrand=1');
        }else{
            $pounit = DB::SELECT('SELECT * FROM unit_pull_outs WHERE isBrandNew=0 AND POUBrand=1');
        }
    ...
    public function deleteCU(Request $request){
        ...
        $cunit = DB::SELECT('SELECT unit_confirms.id, unit_confirms.POUID, unit_confirms.CUTransferDate, unit_confirms.CUTransferRemarks, unit_confirms.CUTransferStatus, unit_confirms.CUTransferArea, unit_confirms.CUTransferBay,
                            ...
                            FROM unit_confirms
                            INNER JOIN unit_pull_outs on unit_pull_outs.id = unit_confirms.POUID
                            WHERE unit_pull_outs.POUBrand = 1 AND unit_confirms.CUDElTransfer = 0
                        ');
        if(count($cunit)>0){
            foreach ($cunit as $CU) {
    ...
    public function sortConfirm(Request $request){
        ...
        if($UStatus == 'cuAllStatus'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuWFRU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=1 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuURU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=2 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuUGU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=3 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuSeU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=4 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuFScU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=5 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuFSaU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=6 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuWP'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=7 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuWBO'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=8 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuWSB'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=9 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuStU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=10 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuRU'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=11 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuWFM'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=12 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuWFP'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=13 AND CUDelTransfer=0 AND POUBrand=1');
        }else if($UStatus == 'cuDP'){
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUTransferStatus=14 AND CUDelTransfer=0 AND POUBrand=1');
        }else{
            $cunit = DB::SELECT('SELECT * FROM unit_confirms LEFT JOIN unit_pull_outs on unit_confirms.POUID = unit_pull_outs.id WHERE CUDelTransfer=1 AND POUBrand=1');
        }
