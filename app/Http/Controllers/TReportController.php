...
class TReportController extends Controller
{
    ...
    public function sortPullOut(Request $request){
        ...
        if($UStatus == 'pouRadioPOU'){
            ...
            $pounit = DB::SELECT('SELECT * FROM unit_pull_outs WHERE POUStatus=2 AND isBrandNew=0 AND POUBrand=1');
        }else if($UStatus == 'pouRadioDU'){
            $pounit = DB::SELECT('SELECT * FROM unit_pull_outs WHERE POUStatus=3 AND isBrandNew=0 AND POUBrand=1');
        }else{
